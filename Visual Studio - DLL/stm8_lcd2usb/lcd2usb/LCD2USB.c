...
#include "lusb0_usb.h"
#include <string.h>
#include <stdbool.h>	// declare bool for C

// return result string
static char result[80]="";

// driver name
#define DRIVER_NAME "STM8 LCD2USB Display DLL V0.14"
...
char * __stdcall  DISPLAYDLL_Init(const BYTE x_size, const BYTE y_size, const unsigned char *startparam, bool *ok)
{
	...
	if (handle == NULL) {
		*ok = FALSE;
		strcpy_s(result,sizeof(result), "no USB2LCD-Device found");
		...
		if (controllers != -1) {
			if (!controllers) {
				*ok = FALSE;
				strcpy_s(result,sizeof(result), "no controllers found");
				controllers = 0;  // don't access any controllers
			} else {
				*ok = FALSE;
				strcpy_s(result,sizeof(result), "unable to read installed controllers");
				controllers = 0;  // don't access any controllers
			}
			...
			lcd_clear();
			*ok = TRUE;
			strcpy_s(result,sizeof(result), "OK");
		}
	}
	return result;
}
...
unsigned short __stdcall DISPLAYDLL_ReadKey(void)
{
  ...
  if(keymask > 0) 
	// return (keymask & 3) + '0';
	  return keymask + '0';			// removing the mask and let the firmware decide
  else
	return 0xFF00;
}
